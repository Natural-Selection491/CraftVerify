...
namespace NaturalSelection.UserManagement.DisableAccount
{
    ...
    { 
        ...
        private string CraftDisableSQLCommand(Dictionary<string, object> newValues)
        {
            try
            {
        ...
        public bool DisableAccount(string userID)
        {
            try
            {
                ...
                else
                {
                    ...
                    var rowsAffected = _sqlServerDAO.InsertAttribute(connString, CraftDisableSQLCommand(changeToEnable));
                    if (rowsAffected > 0)
                    {
                        return true;
                    }
